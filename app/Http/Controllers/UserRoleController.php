        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                    ...
                    ->addColumn('action', function($data){
                        $btn = ' <a href="'.url('edit/user/role').'/'.$data->id.'" class="mb-1 btn-sm btn-warning rounded d-inline-block"><i class="bi bi-pencil-square"></i></a>';
                        $btn .= ' <a href="javascript:void(0)" data-toggle="tooltip" data-id="'.$data->id.'" data-original-title="Delete" class="btn-sm btn-danger rounded d-inline-block deleteBtn"><i class="bi bi-trash"></i></a>';
                        return $btn;
                    })
                    ->rawColumns(['action'])
                    ->make(true);
    ...
    public function viewUserRolePermission(Request $request){
        if ($request->ajax()) {
            ...
            return Datatables::of($data)
                    ...
                    ->addIndexColumn()
                    ->addColumn('action', function($data){
                        $btn = ' <a href="'.url('/assign/role/permission')."/".$data->id.'" class="btn-sm btn-warning rounded"><i class="bi bi-check2-circle" style="margin-right: 2px; font-size: 14px;"></i> Assign</a>';
                        return $btn;
                    })
