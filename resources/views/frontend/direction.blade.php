...
@section('content')
    ...
    <!-- Breadcrumbs Area -->
    <section class="breadcrumbs-area" style="@if($instructionConfig && $instructionConfig->priority == 1) background-image: url('{{url($instructionConfig->background_image)}}'); @else background-color: {{$instructionConfig->background_color}} !important; @endif">
        <div class="container">
            <div class="row justify-content-center">
                <div class="col-lg-8 col-md-8 col-12">
                    <div class="breadcrumbs-content">
                        <h3 class="breadcrumbs-title">{{ App::currentLocale() == 'en' ? $instructionConfig->page_title : $instructionConfig->page_title_bn }}</h3>
                        ...
                        <div class="single-faq-widget">
                            <h2 class="accordion-header" id="headingOne">
                                <button class="accordion-button @if($sl > 1) collapsed @endif" type="button" data-bs-toggle="collapse"
                                    ...
                                    <h3>{{ App::currentLocale() == 'en' ? $instruction->question : $instruction->question_bn }}</h3>
                                </button>
                            </h2>
                            <div id="collapseOne{{$sl}}" class="accordion-collapse collapse @if($sl == 1) show @endif" aria-labelledby="headingOne"
                                ...
                                <div class="faq-inner-body">
                                    {!! App::currentLocale() == 'en' ? $instruction->answer : $instruction->answer_bn !!}
                                </div>
                            </div>
                        </div>
                        @php $sl++; @endphp
