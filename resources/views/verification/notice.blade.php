                ...
                <div class="col-lg-6 col-xl-4 col-md-8 col-12">
                    <div class="auth-card verifyOTP-card">
                        <div class="auth-card-head">
                            ...
                            <h4 class="auth-card-head-title">{{ __('label.verify_otp') }}</h4>
                            <p class="auth-card-head-title-text">
                                {{ __('label.six_digit_code_sent') }}
                                <span
                                    class="otp-number d-block">"{{ Auth::user()->email ? Auth::user()->email : Auth::user()->contact }}"</span>
                                {{ __('label.enter_code_to_verify') }}
                            </p>
                        </div>
                        <form class="auth-card-form" action="{{ url('user/verify/check') }}" method="post">
                            @csrf
                            <div class="form-group">
                                <label>{{ __('label.enter_code') }}</label>
                        ...
                        <div class="auth-card-bottom">
                            <p class="OTP-send-again m-0">
                                {{ __('label.didnt_receive_sms') }} <a
                                    href="{{ url('user/verification/resend') }}">{{ __('label.send_again') }}</a>
                            </p>
                        </div>
                    </div>
                </div>
